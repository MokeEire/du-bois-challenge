...
{:else}
  <footer class="bottom">
    ...
    <span style="margin-bottom:8px;">© Mark Barrett {year}.</span>
    <div class="about-links">
      {#each links as { title, url, target, icon }}
        <div class="link-item">
          <a href={url} {target}
            ><span class={icon}></span><span>{title}</span></a
          >
        </div>
      {/each}
    </div>
...
<style>
  ...
  .link-item {
    flex-basis: calc(
      33% - 10px
    ); /* Adjust the percentage for more or fewer items */
    text-align: center;
    opacity: 0.7;
    filter: grayscale(1);
    transition:
